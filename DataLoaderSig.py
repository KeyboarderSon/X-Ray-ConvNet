...
class DataLoader:
	...
	def load_train_data(self):
		...
		if self.undersample:
			...
			label_sig = [x[0] for x in t_label]
			train_images = train_data.apply(lambda x: '../preprocessed256/' + x[0]).values[samples_to_train]#'../database_preprocessed/'
		else:
			train_images = train_data.apply(lambda x: '../preprocessed256/' + x[0]).values[:self.ntrain]
		return (train_images, label_sig)
	...
	def load_validation_data(self):
		...
		if self.undersample:
			...
			label_sig = [x[0] for x in label]

			#label = np_utils.to_categorical(label, num_classes=3)
			val_images = val_data.apply(lambda x: '../preprocessed256/' + x[0]).values[samples_to_val]#'../database_preprocessed/'
		else:
			val_images = val_data.apply(lambda x: '../preprocessed256/' + x[0]).values[:self.nval]
		return (val_images, label_sig)
	...
	def load_test_data(self):
		...
		if self.undersample:
			...
			#label = np_utils.to_categorical(label, num_classes=3)
			test_images = test_data.apply(lambda x: '../preprocessed256/' + x[0]).values[samples_to_test]#'../database_preprocessed/'
		else:
			test_images = test_data.apply(lambda x: '../preprocessed256/' + x[0]).values[:self.ntest]
		return (test_images, label_sig)
